...
class VenvPluginConfig(confu.schema.Schema):
    ...
    pipfile = confu.schema.Str(help="path to Pipfile",
                               default="{{ctx.home}}/Pipfile")
...
class VenvPlugin(command.CommandPlugin):
    ...
    def add_arguments(cls, parser, plugin_config):
        ...
        op_build_parser = sub.add_parser("build", help="build virtualenv",
                                         parents=[install_parser])

        op_sync_parser = sub.add_parser("sync",
        ...
        op_copy_parser.add_argument("source", nargs="?", type=str, help="venv source location")
        op_copy_parser.add_argument("output", nargs="?", type=str, help="venv output location")
    ...
    def build(self, **kwargs):
        ...
        """
        command = ["ctl_venv_build {} {}".format(
            self.output, self.python_version)]
    ...
    def sync(self, **kwargs):
        ...
        if not self.venv_exists():
            self.build(**kwargs)
        command = ["ctl_venv_sync {} {}".format(
            self.output, self.pipfile)]
        self._run_commands(command, **kwargs)
