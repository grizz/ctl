...
class ReleasePlugin(command.CommandPlugin):
    ...
    def add_arguments(cls, parser, plugin_config):
        ...
        op_release_parser = sub.add_parser("release", help="execute release",
                                           parents=[shared_parser])

        op_release_parser.add_argument("--dry", action="store_true",
                                       help="Do a dry run (nothing will be uploaded)")

        op_validate_parser = sub.add_parser("validate", help="validate release",
                                            parents=[shared_parser])
